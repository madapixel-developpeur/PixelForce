...
class AgentInscriptionController extends AbstractController
{
     ...
     */
    public function inscriptionAgent(Request $request, SecteurRepository $secteurRepository, CountryService $countryService)
    {

        $user = new User();
        $ambassador_username = $request->get('parrain', '');
        if (trim($ambassador_username) === '')
            $ambassador_username = null;
        ...
        if ($parrain) {
            $user->setAmbassadorUsername($parrain->getUsername());
        }
        ...
        try {
            ...
            if ($form->isSubmitted() && $form->isValid()) {
                $parrain = $this->getParainByUsername($user->getAmbassadorUsername());
