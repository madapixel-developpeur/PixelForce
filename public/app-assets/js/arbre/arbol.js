  ...
  function drawOrganizationChart(params) {
    ...
    var svg = d3.select(attrs.selector)
      ...
      .call(zoomBehaviours)
      .append("g")
      .attr("transform", "translate(" + attrs.width / 3.5 + "," + 100 + ")");
    ...
    function update(source, param) {
      ...
      .attr("data-node-group-id",function(d){
        ...
        d3.selectAll(".nodeRepresentsCurrentUser")
        .each(function(d) {
          d3.select(this.parentNode).append("rect")
          .attr("width", 26)
          .attr("height", 20)
          .attr("class", "node-collapse-right-rect yo")
          .attr("x", -25)
          .attr("y", 0)
          .attr("rx", 3)
        });
        ...
        .each(function(d) {
          ...
          .attr("style", "fill: white; font-size: 15px; font-weight: 700;")
          .text("Yo")
        });
    ...
    var collapsibles =
        ...
        .attr('class', 'node-collapse')
        .attr('x', attrs.nodeWidth - 14)
        .attr('y', attrs.nodeHeight - 14)
        .attr('rx', 3)
        .attr('width', 14)
        .attr("", setCollapsibleSymbolProperty);

        // collapsibles.attr("fill", "#42b2ff");

      //hide collapse rect when node does not have children
      collapsibles.attr("height", function(d) {
    ...
    //     .attr("height", attrs.collapseCircleRadius)
      ...
      if(param && param.locate){
        ...
        var new_x = (-x + (window.innerWidth / 3.5 ));
        var new_y = (-y + (window.innerHeight / 3.5 ));
      ...
      function tooltipContent(item) {
        ...
        strVar += "      <h4 class=\"tags-wrapper\">";
        strVar += "        ";
        // strVar += "        <span class=\"title\"><i class=\"fa fa-tags\" aria-hidden=\"true\"><\/i><\/span><ul class=\"tags\">" + getTagsFromCommaSeparatedStrings(item.tags) + "<\/ul>";
        strVar += "      <\/h4> <\/div>";
    ...
    function reflectResults(results) {
      var htmlStringArray = results.map(function(result) {
        ...
        strVar += "               <div class=\"image-wrapper col-4\">";
        strVar += "                 <img class=\"image\" src=\"" + result.imageUrl + "\"\/>";
        strVar += "               <\/div>";
        strVar += "               <div class=\"description col-8\">";
        strVar += "                   <p class=\"name\">" + result.name + "<\/p>";
        strVar += "      <p class=\"name\"> " + (parseInt(item.isStarMember) === true) ? '⭐ Star member ⭐' : '' + "<\/p>";
        strVar += "                   <p class=\"position-name\">" + result.positionName + "<\/p>";
        strVar += "                   <p class=\"area\">" + result.area + "<\/p>";
        strVar += "               <\/div>";
        strVar += "            <\/div>";
        strVar += "            <div class=\"buttons\">";
        strVar += "              <button class='btn-search-box btn-action btn-locate' onclick='params.funcs.locate("+result.uniqueIdentifier+")'>Localizar <\/button>";
        strVar += "            <\/div>";
        strVar += "          <\/a>";
        strVar += "        <\/div>";
      ...
      set('.user-search-box .result-header', "Resultados: " + htmlStringArray.length);
    ...
    function searchUsers() {
      ...
      d3.selectAll('.user-search-box')
        ...
        .duration(250)
        .style('width', '280px')
        .style('box-shadow','-3px 0px 6px #00000029')
    }
    ...
    function expandAll() {
      expand(attrs.root);
      update(attrs.root);
      show(['.btn-action.btn-back', '.btn-action.btn-fullscreen']);
    }
    ...
    function toggleFullScreen() {
      ...
      if ((document.fullScreenElement && document.fullScreenElement !== null) ||
        ...
        d3.select(params.selector + ' svg').attr('width', "100%").attr('height', 560);
